...
const CONTENT_DIR = path.join(process.cwd(), "projects");
...
function generateProjects(): void {
    ...
    if (!fs.existsSync(CONTENT_DIR)) {
        console.error(`Error: ${CONTENT_DIR} does not exist`);
        process.exit(1);
    }
    ...
    fs.writeFileSync(OUTPUT_FILE, JSON.stringify(projects, null, 4), "utf-8");
    ...
    console.log(`  - ${projects.filter((p) => p.featured).length} featured`);
