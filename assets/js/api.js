...
async function load() {
    const data = await fetch('https://api.sylvain.pro/v2/website').then(res => res.json());
    ...
    const updateStats4 = () => {
        ...
        if (stats4 && title) {
            ...
            stats4.innerHTML = titles[i].stats === 'projects'
                ...
                : `<a onlick='window.location.href="https:/\/github.com/20syldev?tab=overview&from=2025-01-01#sr-footer-heading"' style='color: var(--bg-invert)'>${stats[titles[i].stats]}</a>`;
        }
        i = (i + 1) % titles.length;
    };
